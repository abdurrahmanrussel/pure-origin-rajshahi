  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ 
        error: 'User not found. Please register or sign up.',
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    const formattedUsers = users.map(user => ({
      ...
      isActive: user.fields.IsActive === true,
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ 
        error: 'User not found. Please register or contact support.' 
      })
    }
...
// Validation for resend verification by email
export const resendVerificationByEmailValidation = [
  body('email')
    .trim()
    .notEmpty()
    .withMessage('Email is required')
...
// Resend verification by email (for unverified users who can't login)
export const resendVerificationByEmail = async (req, res) => {
  try {
    ...
    // Always return success (even if user doesn't exist or already verified) to prevent email enumeration
    if (!user) {
      return res.json({ 
    ...
    // Send verification email
    try {
      await emailService.sendEmailVerificationEmail(user.fields.Email, verificationToken, user.fields.Name)
...
export default {
  ...
  resendVerificationByEmailValidation,
