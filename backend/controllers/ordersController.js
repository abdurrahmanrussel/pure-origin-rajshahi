    ...
    const enrichedOrders = await Promise.all(
      orders.map(async (order) => {
        ...
        return {
          ...
          paymentMethod: order.fields['Payment Method'] || 'N/A',
          transactionId: order.fields['Transaction ID'] || 'N/A',
          offerNumber: order.fields['Offer Number'] || 'N/A',
...
export const getUserOrders = async (req, res) => {
  try {
    console.log('[getUserOrders] Fetching orders for user ID:', req.user.id)
    
    // Get orders by UserID field (Airtable Record ID) - PRIMARY METHOD
    let orders = await orderService.getOrdersByUserId(req.user.id)
    
    // Fallback: If no orders found by UserID, try by email (for historical data)
    if (!orders || orders.length === 0) {
      console.log('[getUserOrders] No orders by UserID, trying email fallback for:', req.user.email)
      orders = await orderService.getOrdersByEmail(req.user.email)
    }
